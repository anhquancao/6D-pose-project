...
def loss_no_conf(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, num_point_mesh, sym_list):
...
class LossNoConf(_Loss):
    ...
    def __init__(self, num_points_mesh, sym_list):
        super(LossNoConf, self).__init__(True)
        self.num_pt_mesh = num_points_mesh
        self.sym_list = sym_list

    def forward(self, pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine):

        return loss_no_conf(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, self.num_pt_mesh, self.sym_list)
...
def loss_conf(pred_loss, pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, num_point_mesh, sym_list):
    knn = KNearestNeighbor(1)
    bs, num_p, _ = pred_c.size()
...
class LossConf(_Loss):

    def __init__(self, num_points_mesh, sym_list):
        super(LossConf, self).__init__(True)
        self.num_pt_mesh = num_points_mesh
        self.sym_list = sym_list

    def forward(self, pred_loss, pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine):

        return loss_conf(pred_loss, pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, self.num_pt_mesh, self.sym_list)
